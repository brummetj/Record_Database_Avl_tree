 ...
 * one in any sub-tree.
 */
class AVLTree : public RecordType  {
...
public:
    ...
    //Adding Vector Functions.
    std::vector<RecordType> &addtoVector(AVLNode *node, std::vector<RecordType> &vec);
    std::vector<RecordType> &addtoVector(std::vector<RecordType> &vec);
    ...
    RecordType &locate(RecordType existingData);
...
protected:
    ...
    bool search(const std::string& s, AVLNode *& tree);
